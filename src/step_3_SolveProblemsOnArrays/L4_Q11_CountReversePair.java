...
public class L4_Q11_CountReversePair {
    ...
    public static void mergeSortCount(List<Integer> array_list, int start , int mid, int end){
        int left = start;
        int right = mid + 1;
        List<Integer> temp = new ArrayList<>();

        while (left <= mid && right <= end){
            if(array_list.get(left) <= array_list.get(right)) temp.add(array_list.get(left++));
            else temp.add(array_list.get(right++));
        }
...
//
//        while(left <= mid && right <= end){
//            if(array_list.get(left) > 2* array_list.get(right)) {
//                count += mid - left + 1;
//                right++;
//            }
//            else left++;
...
//    }
    ...
    public static int calculatePairCount(List<Integer> array_list, int start , int mid, int end) {
        ...
        for (int i = start; i <= mid; i++) {
            while (right <= end && array_list.get(i) > 2 * array_list.get(right)) right++;
            cnt += (right - (mid + 1));
        }
        return cnt;
    }
    ...
    public static int countReversePair(List<Integer> array_list, int start , int end) {
        ...
        count += calculatePairCount(array_list, start, mid , end);
        mergeSortCount(array_list , start , mid , end);
